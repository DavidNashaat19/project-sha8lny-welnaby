...
# ---------------------------------------------------------------------------
# Arabic stop-words (hardcoded — no network/nltk.download() required)
# ---------------------------------------------------------------------------
ARABIC_STOPWORDS = {
    "في", "من", "إلى", "على", "عن", "مع", "هذا", "هذه", "هو", "هي",
    "هم", "أن", "إن", "كان", "كانت", "يكون", "لا", "ما", "لم", "لن",
    "قد", "التي", "الذي", "التي", "الذين", "كل", "بعض", "بين", "حتى",
    "أو", "و", "ف", "ب", "ل", "ك", "وقد", "وهو", "وهي", "وهم",
    "عند", "بعد", "قبل", "خلال", "حول", "تم", "يتم", "سيتم",
    "مطلوب", "يرجى", "يجب", "نريد", "نبحث", "نحتاج",
    "المشروع", "العمل", "الخبرة", "للعمل", "مشروع",
    "ال", "الى", "انا", "نحن", "انت",
}

# ---------------------------------------------------------------------------
# English stop-words (compact IR-focused list)
# ---------------------------------------------------------------------------
ENGLISH_STOPWORDS = {
    ...
    "how", "when", "where", "why", "all", "any", "few", "more", "most",
    "other", "some", "such", "than", "too", "very", "just", "about",
    "above", "after", "before", "between", "into", "through", "during",
    "including", "without", "project", "work", "need", "looking", "want",
    "experience", "please", "required", "must", "good", "great",
}
...
def process_row(row: dict) -> dict:
    ...
    """
    # --- Combine text fields ---
    desc = str(row.get("full_description") or row.get("description_snippet") or "")
    ...
    budget_raw = (
        str(row.get("budget_min") or "") + " " +
        str(row.get("budget_max") or "")
    ).strip()
    ...
    return {
        ...
        "skills_clean":      [
            " ".join(tokenize_and_filter(
                lowercase_english(normalize_arabic(remove_noise(s)))))
            for s in (row.get("skills") or [])
        ],
        "category_clean":    lowercase_english(
            normalize_arabic(remove_noise(str(row.get("category") or "")))),
