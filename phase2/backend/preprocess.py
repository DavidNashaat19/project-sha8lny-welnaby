...
# and combined with custom domain-specific freelance terms.
# ---------------------------------------------------------------------------
...
ARABIC_STOPWORDS = NLTK_ARABIC_STOPWORDS | CUSTOM_ARABIC_STOPWORDS
...
def main():
    ...
    # Treatment strategy:
    #   • title / full_description / category  → fill with empty string
    #   • budget_min / budget_max              → fill with column median
    #   • skills                               → fill with empty list
    #   • posted_date / url / platform         → fill with 'Unknown'
    text_cols = ["title", "full_description", "description_snippet", "category"]
    for col in text_cols:
        if col in df_check.columns:
            df_check[col].fillna("", inplace=True)

    numeric_cols = ["budget_min", "budget_max"]
    for col in numeric_cols:
        if col in df_check.columns:
            median_val = pd.to_numeric(df_check[col], errors="coerce").median()
            df_check[col].fillna(median_val if not pd.isna(median_val) else 0, inplace=True)
    ...
    for col in ["platform", "url", "posted_date", "budget_currency", "budget_type"]:
        if col in df_check.columns:
            df_check[col].fillna("Unknown", inplace=True)
